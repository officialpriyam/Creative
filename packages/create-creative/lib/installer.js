...
async function createEnvFile(projectPath, sandbox, answers) {
  let envContent = '# Creative Configuration\n\n';
  
  // Sandbox provider
  envContent += `# Sandbox Provider\n`;
  envContent += `SANDBOX_PROVIDER=${sandbox}\n\n`;
  
  envContent += `# LOCAL WEB SCRAPING\n`;
  envContent += `# URL scraping and screenshots are handled locally with Playwright.\n`;
  envContent += `# Optional: set a self-hosted SearXNG endpoint to enable search terms.\n`;
  if (answers.searxngUrl) {
    envContent += `SEARXNG_URL=${answers.searxngUrl}\n\n`;
  } else {
