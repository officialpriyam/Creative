...
function AISandboxPage() {
  ...
  const [optimizedPromptInput, setOptimizedPromptInput] = useState('');
  ...
  const isLikelySourceUrl = (value: string) => {
  ...
  useEffect(() => {
    ...
    const initializePage = async () => {
      ...
      const storedCreativePrompt = sessionStorage.getItem('creativePrompt');
      const storedOptimizedPrompt = sessionStorage.getItem('optimizedPrompt');
      const storedReferenceImage = sessionStorage.getItem('referenceImage');
      const storedReferenceImageName = sessionStorage.getItem('referenceImageName');
      ...
      if (storedUrl || storedCreativePrompt) {
        ...
        setHomePromptInput(storedCreativePrompt || '');
        setOptimizedPromptInput(storedOptimizedPrompt || storedCreativePrompt || '');
        setReferenceImage(storedReferenceImage || '');
        setReferenceImageName(storedReferenceImageName || '');
      ...
      try {
        ...
        if ((storedUrl || storedCreativePrompt) && isMounted) {
  ...
  useEffect(() => {
    if (!showHomeScreen && homeUrlInput && isLikelySourceUrl(homeUrlInput) && !urlScreenshot && !isCapturingScreenshot) {
  ...
  useEffect(() => {
    ...
    if (autoStart === 'true' && !showHomeScreen && (homeUrlInput || homePromptInput)) {
      sessionStorage.removeItem('autoStart');
      // Small delay to ensure everything is ready
      setTimeout(() => {
        console.log('[generation] Auto-starting generation');
        startGeneration();
      }, 1000);
    }
  }, [showHomeScreen, homeUrlInput, homePromptInput]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    if (shouldAutoGenerate && (homeUrlInput || homePromptInput) && !showHomeScreen) {
      // Reset the flag
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [shouldAutoGenerate, homeUrlInput, homePromptInput, showHomeScreen]);
...
//   };
  ...
  const startPromptOnlyGeneration = async () => {
    const userPrompt = (optimizedPromptInput || homePromptInput || homeContextInput).trim();
    ...
    setShowLoadingBackground(true);
    setChatMessages([]);
    addChatMessage('Planning your app from the prompt...', 'system');

    if (referenceImage) {
      setUrlScreenshot(referenceImage);
      setIsPreparingDesign(true);
      setTargetUrl(referenceImageName || 'Reference image');
    }
    ...
    setTimeout(async () => {
      ...
      setUrlInput('');
      setUrlOverlayVisible(false);
      setUrlStatus(['Optimizing prompt...', 'Generating React app...']);

      try {
        const createdSandbox = await sandboxPromise;
        const prompt = `Build a complete React application from this optimized product brief.
...
The user attached "${referenceImageName}". The prompt optimizer has already incorporated any visual details it could infer. Use the brief as the source of truth and create a polished result that matches the requested mood, layout, and visual hierarchy.` : ''}

IMPORTANT INSTRUCTIONS:
- Build what the user asked for, not a clone template.
- Create a complete, working React app with visible content immediately.
- Use Tailwind CSS for styling.
- Create all imported components.
- Include realistic content, empty states, responsive behavior, and polished UI states where relevant.
- Make the result feel like a production-quality first pass from a web app maker.`;

        setConversationContext(prev => ({
          ...prev,
          scrapedWebsites: referenceImageName
            ? [...prev.scrapedWebsites, {
                url: `reference:${referenceImageName}`,
                content: { prompt: userPrompt, referenceImageName },
                timestamp: new Date()
              }]
            : prev.scrapedWebsites,
          currentProject: 'Prompt-generated app'
        }));
        ...
        const activeSandbox = createdSandbox || sandboxData;
        const aiResponse = await fetch('/api/generate-ai-code-stream', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
        ...
        addChatMessage('App generated from your prompt.', 'system');
        if (explanation?.trim()) {
          addChatMessage(explanation, 'ai');
        }
        ...
        addChatMessage('Your prompt-built app is ready. You can now ask for edits in the chat.', 'ai', {
          scrapedContent: { prompt: userPrompt, referenceImageName },
          generatedCode
        });

        setHomePromptInput('');
        setOptimizedPromptInput('');
        setReferenceImage('');
  ...
  const startGeneration = async () => {
    if (!homeUrlInput.trim() && (homePromptInput.trim() || optimizedPromptInput.trim())) {
      await startPromptOnlyGeneration();
      return;
    }
