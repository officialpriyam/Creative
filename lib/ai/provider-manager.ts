...
function getEnvDefaults(provider: ProviderName): { apiKey?: string; baseURL?: string } {
  ...
  switch (provider) {
    case 'openai':
      return { apiKey: configured(process.env.OPENAI_API_KEY), baseURL: process.env.OPENAI_BASE_URL };
    case 'anthropic':
      // Default Anthropic base URL mirrors existing routes
      return { apiKey: configured(process.env.ANTHROPIC_API_KEY), baseURL: process.env.ANTHROPIC_BASE_URL || 'https://api.anthropic.com/v1' };
    case 'groq':
      return { apiKey: configured(process.env.GROQ_API_KEY), baseURL: process.env.GROQ_BASE_URL };
    case 'google':
      return { apiKey: configured(process.env.GEMINI_API_KEY), baseURL: process.env.GEMINI_BASE_URL };
    default:
      return {};
  }
}
